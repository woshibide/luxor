...
MAX_DIMENSION = 1080    # maximum width or height in pixels
...
def find_optimal_quality(image_path, max_size, min_quality=MIN_QUALITY, max_quality=INITIAL_QUALITY, prepared_img=None):
    ...
    if suffix not in ['.jpg', '.jpeg', '.png', '.webp', '.tif', '.tiff', '.heic']:
...
def process_directory(directory, recursive=RECURSIVE):
    ...
    for item in entries:
        ...
        if os.path.isfile(item_path) and item.lower().endswith(('.png', '.jpg', '.jpeg', '.tif', '.tiff', '.heic')):
            try:
